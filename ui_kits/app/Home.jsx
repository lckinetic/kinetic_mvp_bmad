...
function PainPoint({ children }) {
  return (
    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '10px 0', borderBottom: `1px solid ${KColors.border}` }}>
      <div style={{ width: 4, height: 4, borderRadius: '50%', background: KColors.error, marginTop: 6, flexShrink: 0 }}/>
      <div style={{ fontSize: 13, color: KColors.fg2, lineHeight: 1.5 }}>{children}</div>
    </div>
  );
}
...
function SolutionPoint({ children }) {
  return (
    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '10px 0', borderBottom: `1px solid ${KColors.border}` }}>
      <div style={{ width: 4, height: 4, borderRadius: '50%', background: KColors.success, marginTop: 6, flexShrink: 0 }}/>
      <div style={{ fontSize: 13, color: KColors.fg2, lineHeight: 1.5 }}>{children}</div>
    </div>
  );
}

function ScreenCard({ step, title, icon, description, hint, onClick }) {
...
function UseCaseCard({ segment, useCase, prompt, workflow }) {
  return (
    <div style={{ background: KColors.overlay, border: `1px solid ${KColors.border}`, borderRadius: 10, overflow: 'hidden' }}>
      ...
      <div style={{ padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        {workflow && (
          <div style={{ fontSize: 11, color: KColors.fg3, lineHeight: 1.6 }}>
            <span style={{ color: KColors.fg2, fontWeight: 500 }}>Workflow: </span>{workflow}
          </div>
        )}
      </div>
    </div>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '40px 32px 80px' }}>
        <div style={{ marginBottom: 56 }}>
          ...
          <h1 style={{ fontSize: 42, fontWeight: 700, letterSpacing: '-0.03em', color: KColors.fg1, lineHeight: 1.15, marginBottom: 16, maxWidth: 640 }}>
            Digital and crypto operations,<br/>
        ...
        <div style={{ marginBottom: 56 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            <div style={{ background: KColors.overlay, border: `1px solid ${KColors.border}`, borderRadius: 10, padding: 20 }}>
              ...
              <PainPoint>Companies want crypto capabilities without internal development teams</PainPoint>
              <PainPoint>Juggling multiple systems for trading, custody, DeFi, treasury and payments</PainPoint>
              <PainPoint>Complex integrations take months - each one costs $15-25K</PainPoint>
              <PainPoint>Lack of institutional-grade products for smaller players</PainPoint>
              <PainPoint>Manual processes prone to errors and operational inefficiency</PainPoint>
            </div>
            <div style={{ background: KColors.overlay, border: `1px solid ${KColors.border}`, borderRadius: 10, padding: 20 }}>
              ...
              <SolutionPoint>Plug and play backend for trading, treasury and payments operations</SolutionPoint>
              <SolutionPoint>Unified platform managing everything from one dashboard and API</SolutionPoint>
              <SolutionPoint>Pre-built use case flows for instant deployment</SolutionPoint>
              <SolutionPoint>Custom and configurable operating models</SolutionPoint>
              <SolutionPoint>No-code automation for complex workflows with AI assistance</SolutionPoint>
            </div>
          </div>
        </div>
        ...
        <div style={{ marginBottom: 56 }}>
          ...
          <div style={{ position: 'relative', display: 'flex', gap: 12 }}>
            ...
            <ScreenCard
              ...
              hint="Best for: monitoring and audit"
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            ...
            <LayerCard
              ...
              bullets={[
                ...
                'AI assistant - natural language to runnable workflow',
        ...
        <div style={{ marginBottom: 56 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
            <UseCaseCard
              segment="SMEs and Financial Institutions"
              useCase="Target operating model design"
              prompt="I am in the exploratory phase of defining strategy. Here are my goals and requirements."
              workflow="Output: Target Operating Model + proposed solution workflow examples"
            />
            <UseCaseCard
              segment="Crypto-native and non-crypto SMEs"
              useCase="Treasury diversification"
              prompt="I want to execute a specific ETH accumulation strategy and earn yield."
              workflow="Onramp to USDC -> multi-venue ETH trade -> custody at Fireblocks -> optional Lido staking -> stablecoin payout"
            />
            <UseCaseCard
              segment="Fintechs and PSPs"
              useCase="Stablecoin rewards"
              prompt="I want to reward users who deposit more than X."
              workflow="Stablecoin onramp -> track threshold -> conditional logic -> reward payout -> notifications"
            />
          </div>
        </div>
            ...
            <div style={{ display: 'flex', gap: 24 }}>
              <RoadmapStep
                ...
                items={[
                  ...
                  'AI assistant - natural language to workflow',
                  'Sandbox: Banxa, Privy, Coinbase',
                  'Full UI: Workflows, Operations, AI Assistant, Workflow Builder',
                  'FastAPI backend · PostgreSQL',
                ]}
              />
              <RoadmapStep
                phase="6-12 months"
                title="Go Live - US Launch"
                active={false}
                items={[
                  'Connect real accounts and transactions',
                  'Production-grade flows',
                  '3-5 US design partner pilots',
                  'Hire CTO and engineering team',
        ...
        <div style={{ marginTop: 48, paddingTop: 24, borderTop: `1px solid ${KColors.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ fontSize: 11, color: KColors.fg3 }}>September 2025 · Confidential · kinetic.xyz</div>
        </div>
      </div>
    </div>
  );
